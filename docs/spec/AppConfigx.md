...
### `BASE_NAME_GISTS: ClassVar[str]`

`gists` — リポジトリ一覧ファイル (`gists.yaml`) の基本名。
...
| モジュール | 用途 |
|---|---|
| `command_list.py` | `BASE_NAME_SNAPSHOTS`, `BASE_NAME_GISTS` でストア取得 |
| `command_setup.py` | `BASE_NAME_GISTS`, `BASE_NAME_SNAPSHOTS` で初期 DB 出力 |
