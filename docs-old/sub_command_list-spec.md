...
4. `repos.yaml` を新スナップショット内容でマージ更新する。
...
- 取得結果のスナップショット化
- `snapshots.yaml`、`snapshots/<snapshot-id>/snapshot.yaml`、`repos.yaml` の整合的な更新
...
- `repos.yaml` を新スナップショット内容でマージ更新する。
