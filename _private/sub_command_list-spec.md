...
4. `gh repo list` に渡す `--json` は、ユーザ指定にかかわらず少なくとも `visibility` を含むこと。
...
- `--json` で指定されたフィールド列に `visibility` が含まれていない場合は、
  実装は `visibility` を強制的に追加する。
...
- 取得結果の各要素は `visibility` を含み、値は `public` / `internal` / `private` のいずれかであること。
...
1. ユーザを確定する。
2. `gh repo list` を実行する（`--json` に `visibility` を含める）。
3. 新しい回数を採番する。
